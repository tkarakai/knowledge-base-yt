...
export function proxy(request: NextRequest) {
  // The local knowledge workspace has its own layout and no locale/auth providers.
  const kbPath = request.nextUrl.pathname;
  if (kbPath === "/kb" || kbPath.startsWith("/kb/") || kbPath === "/api/kb" || kbPath.startsWith("/api/kb/")) {
    const nonce = btoa(crypto.randomUUID());
    const csp = `default-src 'self'; script-src 'self' 'nonce-${nonce}' 'strict-dynamic'` + (process.env.NODE_ENV === "development" ? " 'unsafe-eval'" : "") + "; style-src 'self' 'unsafe-inline'; img-src 'self' data:; connect-src 'self'; font-src 'self'; frame-ancestors 'none'; object-src 'none'; base-uri 'self'; form-action 'self'";
